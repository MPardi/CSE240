...
void part1(int x) {
    ...
    //Originally was ++m, this preprocess the variable, changed to m++ for correction
    result = macro_1(m++);
    
    printf("macro_1(%d) = %d\n\n", (++x), result);
    
    // Why did this error occur? Please provide the answer in your own words below following "Explanation: "
    printf("Explanation: Because you incremented m before it was ran through the macro with ++m, you want to have it be m++ to increment after it's through macro.\n\n\n"); // (5 points)
}
...
void part2(int x, int y) {
    ...
    printf("In VS : the result of function_1(x, y) = -273 and macro_2(x, y) = -549 \n"); // (5 points)
    printf("In GCC: the result of function_1(x, y) = -273 and macro_2(x, y) = -600 \n\n"); // (5 points)
    ...
    printf("Explanation: Because the two compilers have different orders of operation when a macro is called, it's not compiler independent.\n\n"); //  (5 points)
}
